...
async function main() {
  ...
  // 6.2 Configurações do sistema — aprovação automática de atividades
  // ATIVADA por padrão (decisão de negócio a pedido do usuário). Um
  // ADMIN_MASTER pode desligar em Admin > Configurações, sem deploy.
  await prisma.systemSetting.upsert({
    where: { key: 'AUTO_APPROVE_ACTIVITIES' },
    update: {},
    create: { key: 'AUTO_APPROVE_ACTIVITIES', value: 'true' },
  });
